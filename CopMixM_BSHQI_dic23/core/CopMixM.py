...
from core.initialize_clusters import initialize_clusters_base, initialize_clusters_random_parall, initialize_best_family
...
class CopMixM:
    ...
    def fit(self, X):
        ...
        elif self.init_clust=='fit_best_family':
            clusters=initialize_best_family(self,X)
        else:
        ...
        for i in range(self.n_iter):
            ...
            if self.maximization_type=='IFM':
                ...
                metrics2.append(metric2)
                likelihoods.append(likelihood_sum)
            elif self.maximization_type=='EMP':
                resp = expectation_step_EMP(self,X, clusters)
                self.resp=resp
                opt_max_EMP=self.opt_max_EMP_KDEpy
                maximization_step_EMP_kdepy(self, X, clusters, resp, opt_max_EMP)
    ...
    def predict(self, X):
        ...
        return labels, clusters #self.resp
