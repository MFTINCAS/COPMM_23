# -*- coding: utf-8 -*-
"""
Created on Thu Dec 16 17:41:14 2021
...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb 11 07:12:21 2021
...
def maximization_step_EMP_spline(self,X, clusters,resp, opt_max_EMP):
    ...
    for cluster in clusters:
      ...
      #   bet_k = np.copy(cluster['bet_k'])
        ...
        (kde_pdf,ecdf)=emp_distr(X,al_k,bet_k,resp_nk,index_cluster,k, opt_max_EMP)
...
def fun_EMP1(params, X, cop_k, resp_nk,index_cluster,k):
    n_features=X.shape[1]
    al_k=params[:n_features]
    bet_k = params[n_features:2*n_features]    
   # ce_k = params[2*n_features:]       
    log_k_pdf= np.zeros(X.shape)
    k_cdf= np.zeros(X.shape)
    
    #(log_k_pdf,k_cdf)=emp_distr(X,al_k,bet_k,resp_nk,index_cluster,k)
